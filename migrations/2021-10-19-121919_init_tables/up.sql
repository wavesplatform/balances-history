...
CREATE TABLE balance_history_max_uids_per_height(
    uid BIGINT GENERATED BY DEFAULT AS IDENTITY CONSTRAINT balance_history_max_uids_per_height_pk PRIMARY KEY,
    balance_history_uid BIGINT NOT NULL CONSTRAINT "bhmax_bh_fkey" REFERENCES balance_history (uid) ON DELETE CASCADE,
    asset_id BIGINT,
    address_id BIGINT,
    block_uid BIGINT,
    height INTEGER,
    amount numeric (100,0),
    UNIQUE(asset_id, height, address_id) include(amount)
);

CREATE INDEX balance_history_max_uids_per_height_bh_uid ON balance_history_max_uids_per_height(balance_history_uid);

--select asset_id, address_id, block_uid, max(uid) as balance_history_uid, amount from balance_history group by 1,2,3;

--insert into blocks_microblocks (id, height, block_type, time_stamp, is_solidified) values ('4QTW84UHCiabKSmVd3H6jYJeravBzUjhBPY49dE62ScW', 2978230, 'block', 1644236707256, true);
