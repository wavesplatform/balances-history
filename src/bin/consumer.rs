...
async fn main() -> Result<()> {
    ...
    let start_height =
        match mappers::blocks_microblocks::get_last_safe_height_from_blocks(&db).await {
...
async fn init_db_data(db: &Db) {
    ...
    db.client
        .query(
            "delete from blocks_microblocks where height > coalesce((select min(height) from safe_heights), 0)",
            &[],
        )
        .await
        .unwrap();
